...
class CdbReaderThread(threading.Thread):
    ...
    def run(self):
        dprint('ReaderThread.run()')
        curline = ''
        # read from the pipe
        while True:
            ch = self.pipe.stdout.read(1)
            dprint('ReaderThread.run(): read %s' % (ch))
            if not ch:
                # add a closed event to the queue
                self.queue.put(PipeClosedEvent())
                dprint('ReaderThread.run(): read nothing')
                break
...
class PyCdb:
    ...
    def shell(self):
        print "Dropping to cdb shell type exit"
        PyCdbShell(self).cmdloop()

class PyCdbShell(cmd.Cmd):
    def __init__(self, pycdb):
